...
size_t bptNode::find_pos(const int& key)
{
	size_t i = 0;
...
size_t bptNode::find_pos(const int& key)const
{
	size_t i = 0;
	while (i != keys.size() && key > keys[i]) {
		++i;
...
int bptNode::successor(const int& curr)
{
	if (isLeaf) {
...
bool bptNode::find(const int& key) const
{
	if (isLeaf) {
...
int& bptNode::search(const int& key)
{
	if (isLeaf) {
		for (size_t i = 0; i != keys.size(); i++) {
...
bptNode* bptNode::insert(const int& key, const int& value)
{
	if (isLeaf) {
		size_t pos = find_pos(key);
...
bptNode* bptNode::del(const int& key)
{
	if (isLeaf) {
		size_t pos = find_pos(key);
